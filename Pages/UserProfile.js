...
const UserProfile = ({ navigation }) => {
    ...
    const user = async () => {
        ...
        name = await JSON.parse(name).data.name
        setName(name)
        let email = await AsyncStorage.getItem('users')
        email = await JSON.parse(email).data.email
        setEmail(email)
        let number = await AsyncStorage.getItem('users')
        number = await JSON.parse(number).data.number
        SetNumber(number)
    }
                ...
                <View>
                    ...
                    <View style={{
                        ...
                        <TouchableOpacity style={{
                            ...
                            borderRadius: 7,
                            marginTop: 8
                        }} onPress={()=>navigation.navigate('hospitalbook')}>
                            <Text style={{
                                fontSize: 20,
                                color: "#000",
                                fontWeight: "500"
